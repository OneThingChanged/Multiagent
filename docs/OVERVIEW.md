...
| 멀티 탭 / 분할 | 한 패널에 여러 탭, 한 그룹당 2패널 h/v 분할, 핸들로 크기 조절. 분할된 패널에 새 파트너를 붙이면 이전 분할은 해제되고 새 2패널 그룹으로 교체 |
| 그룹 | 분할로 묶인 세션들이 한 그룹 — 누굴 클릭하든 그룹 전체 표시 |
| 분할 Screen 요약 | 사이드바의 `This PC`/프로젝트 트리 위에 `Screen N (A+B)`를 표시하고, 교차 프로젝트 멤버에도 같은 색 `SN` 배지를 표시 |
| 드래그 앤 드롭 | 탭/사이드바 세션을 5존 드롭(center=탭, 4-edge=2패널 분할 교체)으로 재배치 |
