...
| Remote background notifications | approved PWA clients use VAPID Web Push. The sideloaded Android APK can instead run an account-free Foreground Service with independent workers for multiple registered PC endpoints, receiving privacy-safe completion/question events while its ongoing notification is active |
...
| feature | description |
|---|---|
| File tree sidebar | full project file tree on the right sidebar — project dropdown, pin (📌), **Repository selector for initialized Git submodules (including nested submodules)**, per-repository expansion state persistence, expand/collapse all, git status badges (M/U/A/D colors, folder propagation, event-driven refresh on agent completion/window focus/manual action), Find files search, MD/image/code/HTML type filters that hide folders without matching descendants, context menu (new file/folder, duplicate, rename, trash delete, copy path). Open via the 🗀 button at the window's top-right; open state/width persisted |
| Source Control view | the ⎇ tab of the file tree panel — operates on the repository selected above (main project or submodule), with branch/ahead-behind, Staged/Changes groups (per-file & stage-all), per-file +/− line counts, Commit after entering a message (Ctrl+Enter), and recent commits. A change-count badge is always shown on the tab |
| Document tabs | open a file from the tree/terminal link/QuickOpen → rendered as a tab in the main workspace (md = GFM + highlight, html = isolated Electron browser embedded in that pane with an editable HTTP(S) address bar and in-pane link navigation, images, text = read-only source). Splits/drags/restores in the same tab strip as terminal tabs |
| Agent browser bridge | the isolated HTML browser uses one app-local persistent profile with tab IDs; Codex/Claude can use the managed `multiagent-browser` stdio MCP (`browser_tabs`, navigation, snapshot, screenshot, click/type, and annotation). Hover/click descriptors are bounded and redacted. **영역 선택** copies JSON/HTML/PNG context to the clipboard, while **선택 후 전송** submits it to the associated session ([BROWSER_MCP.md](BROWSER_MCP.md)) |
...
Built-in web server + Cloudflare Tunnel (quick/named, fixed domain possible) + GitHub login + **account approval**. Session list, chat/terminal input, image attachments, local-project Markdown viewing, and short-lived isolated HTML tabs are available in Remote. On Electron desktop, HTML documents open through a dedicated isolated Document Browser so relative CSS, JavaScript, images, fonts, and media retain their project paths without granting the page Electron access. A dedicated Codex/Claude remaining-account-limit tab and an authenticated resumable Android APK download are also available from the external browser/PWA. The native Android shell lives in `mobile/`. Independent viewer (views a different session than the desktop).
...
| Multi-window | the tray/main process owns shared PTYs while every visible window is an equal workspace with its own persisted Screen layout and the full project/session UI. A session can be controlled by one workspace at a time; other windows show "사용 중". Closing all workspaces leaves the tray and PTYs alive, and tray open restores the last workspace |
