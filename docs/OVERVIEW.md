...
| feature | description |
|---|---|
| File tree sidebar | full project file tree on the right sidebar — project dropdown, pin (📌), **Repository selector for initialized Git submodules (including nested submodules)**, per-repository expansion state persistence, expand/collapse all, git status badges (M/U/A/D colors, folder propagation, 10s polling), Find files search, MD/image/code/HTML type filters that hide folders without matching descendants, context menu (new file/folder, duplicate, rename, trash delete, copy path). Open via the 🗀 button at the window's top-right; open state/width persisted |
