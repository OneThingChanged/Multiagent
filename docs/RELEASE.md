...
- endpoint(`tauri.conf.json` → `plugins.updater.endpoints`): `https://github.com/OneThingChanged/Multiagent/releases/latest/download/latest.json`
- 즉 **GitHub의 "Latest" 릴리즈에 첨부된 `latest.json`** 을 읽는다.
...
2. `latest.json` + `.sig` 자산이 릴리즈에 **첨부**돼 있어야 한다
...
# 자산 5개가 다 있나 (setup.exe, .sig, msi, .sig, latest.json)
...
- [ ] 버전 4곳 모두 갱신했나
- [ ] `TAURI_SIGNING_PRIVATE_KEY`(+빈 PASSWORD) 주고 빌드했나 → `.sig` 생성 확인
- [ ] `latest.json`의 signature가 **NSIS setup.exe.sig** 내용인가
- [ ] 릴리즈가 **draft 아님 + Latest 마킹**인가
- [ ] 자산 5개(setup.exe, setup.exe.sig, msi, msi.sig, latest.json)를 다 올렸나
