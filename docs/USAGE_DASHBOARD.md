...
| path | query | returns |
|---|---|---|
| `GET /api/usage` | `refresh=1?`, `period=week\|month\|year`, `year`, `month?`, `week?` | Electron shared PWA payload: cumulative `tokens`, current `periods`, legacy 30-day `timeline`, selected `history`, and account `limits` |
