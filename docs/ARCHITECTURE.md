...
- Remote chat file previews reuse the authenticated document/file APIs. `/api/docs/read` and `/api/files/image` accept an optional agent id, while `/api/files/asset` streams only allowlisted local CSS/images used by sandboxed HTML. Relative paths start at the session's configured folder but must remain inside the synchronized project root after real-path resolution. The client removes scripts, base/meta-refresh navigation, and inlines allowed local HTML/CSS/image dependencies before rendering the sandbox iframe
