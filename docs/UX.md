...
- **Session right-click**: context menu
  ...
  - **Properties** — name, project, tool, status, session ID, creation time, folder, agent ID + edit toggles: **Dangerous mode** (skip-permissions flag), **Alt-screen mode** (Codex only — explicit opt-out that uses Codex's alternate screen/internal history, so the conversation does not remain in terminal scrollback), and per-session **document/HTML parallel workers** for Codex. Worker choices are shown only while their provider is enabled in Settings → Agents. Leave Alt-screen off for `--no-alt-screen`, native scrollback, Ctrl+F, and drag copy. Runtime option changes apply after deactivating and reopening the session; the existing CLI conversation ID is resumed
...
- **New Session**: creates a session inside the active project. Codex sessions can optionally route independent document/Markdown and HTML work to `Codex · Luna Max` or `Claude · Opus` workers. The worker picker only includes tools enabled in **Settings > Agents**; if both Codex and Claude are disabled, the worker section is hidden
...
- **Codex content workers**: worker settings are stored per session. Luna uses invocation-scoped Codex subagent settings (`gpt-5.6-luna`, `max`, at most two concurrent worker threads). Opus is invoked through the installed Claude Code CLI with its existing login, no session persistence, safe mode, an edit-accepting permission mode, and no dangerous bypass flag. The primary Codex session owns task partitioning, avoids concurrent edits to the same file, and verifies the integrated result. Existing sessions do not need to be recreated; deactivate and reopen them once after changing the worker selection
...
- **Multi-select batch bar**: a fixed-height toolbar slot always remains above the groups. It shows disabled `선택 없음` when empty, then `N selected · Stage · Unstage · Discard · ×` after selection. Reserving the slot prevents the first click from shifting rows and breaking a double-click
...
- **Remote PWA** (standard only): mobile remote server·Cloudflare tunnel Start/Stop, GitHub OAuth/Device Flow·Owner, named tunnel (token/hostname/port), account approval management. Status distinguishes working/question/done/waiting/inactive. Desktop/tablet Remote can inspect Screen/pane layouts or an individual Session; phone-width Remote intentionally exposes only individual Sessions. **Documents** renders local-project Markdown in place and opens HTML through a 15-minute isolated new-tab URL so document-relative web assets work without exposing the Remote application origin. Supports mobile home screen install and Web Push notifications. The Android APK can alternatively enable an account-free Foreground Service; its ongoing notification remains visible while completion/question events are delivered from the user's Remote endpoint ([REMOTE.md](REMOTE.md))
- Desktop/tablet Remote Screen panes independently switch between parsed chat and the live terminal, so a multi-pane Screen can mix both views while retaining its selected session tabs. Phone-width Remote redirects Screen routes to an available session
...
- Remote omits the desktop-oriented Monitor overview and global status cards. Its root route resolves to the first Screen on desktop/tablet and an active Session on phone widths, with Documents and Usage as fallbacks
- Remote and the loopback Dashboard share one Usage surface: three calendar cards (today, Monday-based week, current month), a responsive recent-30-day daily token graph, cumulative token-type totals, and account-limit cards. Missing dates remain visible as zero usage
...
- Remote navigation can create a session by selecting its project, enabled AI tool, alias, and optional Dangerous mode. The selected project's folder/SSH configuration is inherited by the desktop coordinator, and the created session is selected after it appears in synchronized state
- Remote Session detail can rename the current session. Creation and rename use authenticated same-origin APIs and are applied only by the coordinator workspace window to prevent duplicate mutations when several desktop windows are open
- The Remote message composer accepts picker-selected, clipboard-pasted, and drag-and-dropped PNG, JPEG, GIF, WebP, and BMP images through the same upload path and limits (up to four files, 8 MB each)
- Multiline input grows to a viewport-relative maximum, then scrolls only inside the text field so the conversation retains as much vertical space as possible
- Remote chat recognizes project-local Markdown, HTML, and image paths in user/assistant messages. Markdown and images open in a viewport-filling preview overlay; HTML navigates to the owning project's Documents screen, selects the matching document, and exposes the isolated new-tab preview action
