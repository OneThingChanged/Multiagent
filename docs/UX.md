...
- **Session right-click**: context menu
  ...
  - **Properties** — name, project, tool, status, session ID, creation time, folder, agent ID + edit toggles: **Dangerous mode** (skip-permissions flag), **Alt-screen mode** (Codex only — when on, runs on the alternate screen so re-render truncation disappears, but the conversation does not remain in terminal scrollback). Toggle changes apply after deactivating and reopening the session
