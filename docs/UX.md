...
- **Remote PWA** (standard only): mobile remote server·Cloudflare tunnel Start/Stop, GitHub OAuth/Device Flow·Owner, named tunnel (token/hostname/port), account approval management. Status distinguishes working/question/done/waiting/inactive. Desktop/tablet Remote can inspect Screen/pane layouts or an individual Session; phone-width Remote intentionally exposes only individual Sessions. **Documents** browses and previews local-project Markdown/HTML files; HTML is sandboxed and scripts are blocked. Supports mobile home screen install, foreground question notifications, and Web Push completion notifications even after the PWA window closes ([REMOTE.md](REMOTE.md))
