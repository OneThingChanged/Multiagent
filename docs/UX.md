...
- Electron 앱 하단에 Codex 계정 한도를 항상 표시한다. 각 항목은 사용률, 작은 진행 막대, 초기화까지 남은 시간을 보여준다
- 상태 바를 누르면 한도 윈도우별 상세 사용률, 갱신 시각, 플랜과 추가 사용량 정보를 팝오버로 확인할 수 있다
- `token_count.rate_limits`가 포함된 로컬 Codex 세션 기록에서 최신 값을 읽고 `usage.db`에 한도별 스냅샷을 보존한다. 세션이 완료되면 자동 갱신하고, **새로고침**은 최근 세션 끝부분을 다시 확인한다
- 기본 Codex 한도와 별도 모델 한도가 동시에 존재하면 각각 독립된 항목으로 표시한다. 70%부터 경고색, 90%부터 위험색을 사용한다
- 계정 한도 메타데이터를 제공하지 않는 도구와 SSH 원격 세션은 현재 하단 한도 표시에 포함하지 않는다. 기존 Usage 대시보드의 로컬 토큰 집계는 그대로 유지된다
