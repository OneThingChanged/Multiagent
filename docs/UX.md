...
- 사이드바의 **Files** 버튼(또는 `Ctrl+Shift+D`)으로 열고 닫음. 열림 상태와 폭은 다음 실행에도 유지됨
- 오버레이가 아닌 레이아웃 형제 — 열면 터미널 영역이 줄어듦. 경계선 드래그로 폭 조절
- 활성 프로젝트 폴더의 **전체 파일/폴더**를 표시 (`node_modules`, `.git`, `target`, `dist`, `build`, `.next`, `.cache`, `.venv`, `__pycache__`, `out` 제외)
