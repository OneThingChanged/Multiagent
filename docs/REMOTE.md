...
- **Documents**: choose a local project and browse its `.md`/`.markdown`/`.html`/`.htm` files in a collapsible folder tree. Generated/internal roots such as `.build-tools`, `.claude`, `.codex`, `.qwen`, `node_modules`, `build`, and `target` are excluded. Search keeps only matching documents and their parent folders visible. On phones the document tree is a dedicated full-screen drawer; selecting a file closes it and gives the preview the full remaining viewport. Markdown renders in place and supports headings, lists, task items, tables, code fences, and relative links to another listed document. HTML shows a launch card and opens in a separate isolated browser tab so its relative CSS, JavaScript, images, fonts, and media retain their normal paths
- **Usage**: a combined token/account view. It supports selectable week/month/year history, past year/month/ISO-week selection, previous/current/next navigation, daily or monthly graphs, selected-period input/output/cache read/write/reasoning details, and Codex/Claude rolling-window remaining percentage, reset time, plan, and extra-usage availability
- **Android app**: stores multiple approved PC profiles and loads the selected PC's PWA in a constrained native WebView. Its connection bar starts collapsed so the chat/terminal keeps nearly the full screen; expand it for back, reload, or PC selection. A user-enabled `remoteMessaging` Foreground Service keeps independent authenticated long-poll connections for every notification-enabled profile and shows profile-labelled completion/question notifications without Firebase, FCM, or Expo Push
...
- The Session list has a **New Session** action. Choose a synchronized project, an AI tool allowed by desktop **Settings > Agents**, an alias, and optional Dangerous mode. The coordinator desktop window creates the session, inherits the project's local/SSH working configuration, starts it through the normal PTY path, and the Remote client selects it when synchronization completes
- Session detail exposes **Rename**. It updates the same persisted desktop session metadata through the coordinator; the browser cannot submit a working folder or raw startup command
...
- Messages entered while a session is recovering/starting stay queued until initialization completes. Raw Terminal input is blocked during that interval. A `SessionStart` or another live agent hook marks the CLI ready; both restored sessions and Remote background activations fall back to ready after 20 seconds if the ready hook was lost, without ever reporting the fallback as active work. The fallback only releases an unchanged `starting`/`recovering` state, so a hook, exit, or deactivation remains authoritative
...
- Browser/PWA completion and interactive-question notifications use Web Push after permission and subscription are enabled. The Android APK instead uses its own Foreground Service while the required ongoing notification is visible. It maintains one parallel monitor per enabled PC profile, continues when the WebView is backgrounded, and can survive removing the task, but force-stop/service termination disables delivery until the user enables it again. Tapping an event notification switches to the originating PC and reopens the matching Session
...
| `GET /api/usage?refresh=1` | cumulative token totals plus current Codex/Claude account-limit snapshots. `refresh=1` starts the throttled live limit refresh in the background; cached limits and local totals return immediately with `refreshPending=true` until it settles |
...
| `POST /api/session/create` | create a session in a synchronized project using an allowed AI tool; same-origin JSON only. Returns `202` with the assigned session id |
| `POST /api/session/rename` | rename an existing synchronized session; same-origin JSON only, 80-character single-line limit |
...
| `GET /api/docs/preview?projectId=...&path=...&agentId=...` | authenticate an HTML-open request and redirect to a 15-minute read-only preview capability URL; `format=json` returns that URL to the native bridge |
| `GET /preview/<token>/<project-path>` | serve allowlisted HTML/CSS/JS/image/font/media files inside the registered project root; every request is real-path checked and sandboxed |
| `GET /api/files/image?projectId=...&path=...&agentId=...` | stream a supported project image for the overlay viewer (25MB limit) |
| `GET /api/files/asset?projectId=...&path=...&agentId=...` | legacy authenticated image/CSS asset stream retained for compatible clients |
...
- Markdown raw HTML is escaped before rendering. HTML is never inserted into the Remote application DOM. An authenticated request issues an unguessable preview URL that expires after 15 minutes, and the document opens in a separate tab under a CSP sandbox without `allow-same-origin`. It cannot read Remote cookies/API responses, submit forms, access devices, or load arbitrary project file extensions. Its own scripts may run only inside that isolated preview and external network connections are blocked.
...
- The service worker caches only the static shell; `/api/**`, `/auth/**`, `/preview/**`, and all POSTs are never cached.
...
- Android monitor tokens are 256-bit random values issued only to an approved same-origin session. Each PC persists only its token's SHA-256 hash in `%LOCALAPPDATA%\com.jintae.multiagent\remote-monitor-devices.json`; the APK stores the per-profile raw token list in one Android Keystore-encrypted payload. There is no token-list/readback API. Logout or profile deletion revokes only the matching PC token, account revocation removes every token owned by that login on that PC, and tokens expire after 180 days. Event bodies contain only project/session display names, a validated agent id, and `작업이 완료되었습니다.` or `응답이 필요합니다.`—never prompts, terminal output, file paths, or tool input
- The native WebView bridge accepts only validated monitor start/stop messages and same-origin `/preview/<43-character-token>/...` external-open requests from the configured Remote origin. Public endpoints require HTTPS. Notification taps accept only the `multiagent://open?agent=...` deep link with a restricted agent id and derive a same-origin Remote route
...
6. Use either client:
   ...
   - Android: tap the `APK` button in the Remote top bar, install the downloaded file, enter a PC name and the same HTTPS tunnel URL, complete the existing GitHub login/approval flow, then tap the notification button and grant Android notification permission. Open the native connection list to add another PC and repeat its login/approval/notification setup. Keep the required “MultiAgent monitoring” notification active while background delivery is wanted; tapping one Remote's notification button again removes only that PC monitor.
...
The Usage tab uses the desktop PC's calendar boundaries: weeks begin Monday, months begin on day one, and ISO week-years are used around calendar-year boundaries. Week/month views keep inactive days as zero-value buckets, while year views return all twelve months. The period controls send the user's pending selection directly to `/api/usage`; rendering the previous response must not overwrite that selection before the request starts. Raw transcript ingestion resumes at each file's saved byte offset, and prior day totals are retained in the PC-local `usage_daily` rollup, so changing periods reads cached daily data and only newly ingested events need aggregation. SSH session accounting remains unsupported. The account section emphasizes the amount **remaining** even though provider APIs report `used_percent`. Codex values come from recent local Codex transcript rate-limit metadata; Claude values come from Claude Code's local OAuth usage endpoint. A provider can therefore be absent until its local session/credential has supplied a snapshot. The refresh button starts that provider lookup in the background while cached limits and local token history remain immediately available; the server protects the upstream check with a 30-second throttle.

Document browsing is local-project only. SSH project files are not transferred in this version. HTML preview URLs preserve project-relative paths for allowlisted CSS, JavaScript, JSON, images, fonts, media, and WebAssembly. Root-relative URLs such as `/assets/file.css` still refer to the Remote server root; reports should use `./assets/file.css` or another document-relative path. Unsupported extensions, files outside the registered project root, and expired preview tokens return an error.
...
Saved PC profiles and the last selected profile reconnect on the next launch. To switch, add, or delete a PC, expand the thin native connection bar and tap the settings button. Only the selected profile owns a WebView, while the Foreground Service runs a separate long-poll worker for every profile with notifications enabled. Notification deep links contain a validated profile id plus agent id so identical agent ids on two PCs cannot collide. The current APK targets Android 7.0 or later and ARM64 devices. The download button is hidden inside the native app itself because it is intended for browser/PWA users who have not installed the app yet.
