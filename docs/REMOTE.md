...
- File preview resolution merges live session status with the synchronized project/session metadata by agent id. This keeps `projectId` and working-folder context available even though the lightweight live-status payload intentionally omits filesystem fields
